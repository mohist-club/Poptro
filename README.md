...
一个 macOS 菜单栏工具：用全局快捷键完成划词翻译、应用启动与自动化操作。

<!-- TODO: 这里放一张实际截图或 GIF,发布前替换 -->

## 功能

- **划词翻译**:选中任意文字,按一下快捷键就弹出翻译结果,支持智谱 GLM / OpenAI / DeepL / Groq / Google AI / 本地 Ollama,可以随时切换
...
- **全局快捷键**:可绑定应用、macOS 快捷指令、系统操作以及 Shell / AppleScript / JXA 脚本
...
- 使用 Ollama 时,翻译内容完全在你本机进行
- API Key 仅保存在本机，并使用与当前 Mac 绑定的 AES-GCM 密钥加密，不会以明文形式落盘或上传

## 从源码编译

需要 macOS 13+ 和当前版本的 Xcode。生成的 App 支持 macOS 13+。
